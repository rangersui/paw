...
export class CDPClient {
  ...
  static async discoverTarget(opts: CDPDiscoveryOptions = {}): Promise<{ id: string; url: string; title: string; webSocketDebuggerUrl: string }> {
    ...
    if (!res.ok) throw new Error(`pet-cursor: CDP discovery failed (${res.status}). Is Chrome running with --remote-debugging-port=${port}?`);
    ...
    if (!pages.length) throw new Error("pet-cursor: no debuggable page found");
    let target = pages[opts.pageIndex ?? 0];
    if (opts.pageUrl) {
      const m = pages.find((p) => p.url.includes(opts.pageUrl!));
      if (!m) throw new Error(`pet-cursor: no page matching "${opts.pageUrl}"`);
      target = m;
    }
    return target;
  }
  ...
  private openWS(url: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.ws.addEventListener("error", (e) => reject(new Error("pet-cursor: WebSocket error")), { once: true });
      this.ws.addEventListener("close", () => {
        this.closed = true;
        this.pending.forEach((p) => p.reject(new Error("pet-cursor: CDP socket closed")));
        this.pending.clear();
      });
  ...
  send<T = any>(method: string, params: Record<string, unknown> = {}): Promise<T> {
    if (this.closed) return Promise.reject(new Error("pet-cursor: CDP socket closed"));
